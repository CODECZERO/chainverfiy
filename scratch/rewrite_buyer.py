...
new_layout_start = """  const walletAddr = user?.stellarWallet || publicKey || ""
...
# Extract the content between the old start and end
# The old end is at line 328
# We'll just replace everything from `return (` up to `<AnimatePresence mode="wait">`
pattern = r"  return \(\n    <div className={`h-screen flex flex-col bg-\[\#030408\].*?<AnimatePresence mode=\"wait\">\n              <motion\.div\n                key={active}\n                initial={{ opacity: 0, y: 40 }}\n                animate={{ opacity: 1, y: 0 }}\n                exit={{ opacity: 0, y: -40 }}\n                transition={{ duration: 0\.5, ease: \[0\.16, 1, 0\.3, 1\] }}\n                className=\"space-y-16\"\n              >"
match = re.search(pattern, content, re.DOTALL)

if match:
    new_content = content[:match.start()] + new_layout_start + content[match.end():]
    
    # Also fix closing tags
    # The old structure had </main> </div>
    # Our new structure has </div> </main>
    # We replace the closing part
    closing_pattern = r"             </motion\.div>\n           </AnimatePresence>\n        </main>\n      </div>"
    new_closing = "             </motion.div>\n           </AnimatePresence>\n          </div>\n        </div>\n      </main>"
    new_content = new_content.replace(closing_pattern, new_closing)
